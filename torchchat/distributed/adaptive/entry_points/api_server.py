...
class TextGenerationServer:
    ...
    async def generate(self, request: Request) -> Response:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid JSON request"
            )
        ...
        results_generator = self.engine.generate(
            request_id, gen_request.prompt, gen_request.sampling_params
        )

        if gen_request.stream:
            return StreamingResponse(
                self._stream_results(results_generator), media_type="text/event-stream"
            )
    ...
    async def _stream_results(
        self, results_generator: AsyncGenerator
    ...
    async def _generate_complete_response(
        self, request: Request, request_id: str, results_generator: AsyncGenerator
    ) -> JSONResponse:
