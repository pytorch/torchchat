...
class ModelRunner:
    ...
    def _initialize_timers(self) -> dict:
        return {
            "prepare_inputs": CpuTimer(CpuOperationMetrics.PREPARE_INPUTS_E2E, rank=self.rank),
            "sampler": CpuTimer(CpuOperationMetrics.SAMPLER_E2E, rank=self.rank),
            "model_execution": CpuTimer(CpuOperationMetrics.MODEL_EXECUTION_E2E, rank=self.rank),
        }
    ...
    def _prepare_inputs(self, seq_metadata_list: List[SequenceMetadata]) -> ModelInput:
        ...
        for seq_metadata in seq_metadata_list:
            if seq_metadata.is_prompt:
                self._process_prompt_sequence(seq_metadata, input_tokens, input_positions, current_prompt_chunk_lens)
            else:
                self._process_generation_sequence(seq_metadata, input_tokens, input_positions)
        ...
        return ModelInput(
            ...
            positions=torch.tensor(input_positions, dtype=torch.long, device=self.device)
        )

    def _process_prompt_sequence(self, seq_metadata, input_tokens, input_positions, current_prompt_chunk_lens):
        prompt_chunk_len = seq_metadata.prompt_chunk_len
        current_prompt_chunk_tokens = seq_metadata.seq.get_next_prompt_chunk_token_ids(prompt_chunk_len)
    ...
    def profile_num_available_blocks(self, block_size: int, gpu_memory_utilization: float) -> Tuple[int, int]:
        torch.cuda.set_device(self.device)
        torch.cuda.empty_cache()
        torch.cuda.reset_peak_memory_stats()

        sampling_params = SamplingParams(top_p=0.99, top_k=self.model.config.vocab_size - 1)
        seq_metadata_list = self._create_profiling_sequences(block_size, sampling_params)
    ...
    def _create_profiling_sequences(self, block_size: int, sampling_params: SamplingParams) -> List[SequenceMetadata]:
        if self.config.scheduler_config.get_type() in {SchedulerType.SARATHI, SchedulerType.SIMPLE_CHUNKING}:
            return self._create_chunking_profiling_sequence(block_size, sampling_params)
        else:
            return self._create_standard_profiling_sequences(block_size, sampling_params)

    def _create_chunking_profiling_sequence(self, block_size: int, sampling_params: SamplingParams) -> List[SequenceMetadata]:
        chunk_size = min(self.config.scheduler_config.chunk_size, self.config.model_config.max_model_len)
        seq = Sequence(
            seq_id=0,
            prompt=None,
        ...
        return [SequenceMetadata(seq=seq, block_table=None, prompt_chunk_len=chunk_size)]

    def _create_standard_profiling_sequences(self, block_size: int, sampling_params: SamplingParams) -> List[SequenceMetadata]:
        max_num_batched_tokens = self.config.scheduler_config.get_max_num_batched_tokens(self.config.model_config.max_model_len)
        ...
        return [
            SequenceMetadata(
                seq=Sequence(
                    seq_id=str(seq_id),
                    prompt=None,
                    prompt_token_ids=[0] * (max_num_batched_tokens // max_num_seqs + (seq_id < max_num_batched_tokens % max_num_seqs)),
                    block_size=block_size,
                    eos_token_id=1,
                    arrival_time=None,
                    sampling_params=sampling_params,
                ...
                block_table=None,
                prompt_chunk_len=max_num_batched_tokens // max_num_seqs + (seq_id < max_num_batched_tokens % max_num_seqs),
            )
