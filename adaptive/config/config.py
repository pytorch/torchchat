...
from torchchat.distributed.types import SchedulerType

primitive_types = {int, str, float, bool, type(None)}

# based on Sarathi-Serve impl:
...
class AdaptiveSchedulerConfig(BaseSchedulerConfig):
    chunk_size: int = field(
        default=512, metadata={"help": "Size of each chunk for Adaptive scheduler."}
    )
    ...
    def get_max_num_batched_tokens(self, max_model_len: int):
        # Never schedule more than chunk_size tokens in one iteration.
